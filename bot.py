...
TOKEN = os.getenv('TELEGRAM_BOT_TOKEN', '8850444640:AAGLTLd7-Fy3Kww0j3oVW6uv_aadrNn6bf8') # Prefer .env
if not TOKEN or TOKEN == "8850444640:AAGLTLd7-Fy3Kww0j3oVW6uv_aadrNn6bf8": # Check if token is missing or placeholder
    # !!! Replace with your actual bot token if not using .env !!!
    TOKEN = "8850444640:AAGLTLd7-Fy3Kww0j3oVW6uv_aadrNn6bf8g" # FALLBACK - Highly recommended to use .env
...
# --- Flask Keep-Alive Server (for Render / Railway deployment) ---
...
@flask_app.route('/')
def flask_index():
    """Simple health page so Render sees an open HTTP port."""
    return "Bot is alive and running!", 200


@flask_app.route('/health')
def flask_health():
    """JSON health endpoint for uptime monitors (UptimeRobot, cron-job.org)."""
    return jsonify({
        "status": "ok",
        "bot": "jwt-token-bot",
...
def run_flask_server():
    ...
    try:
        ...
        print(f"Flask server error: {flask_err}")


def keep_alive():
    """Start the Flask server thread (daemon so it dies with the bot)."""
...
if __name__ == '__main__':
    # Start the Flask keep-alive web server first (needed for Render web service)
    keep_alive()

    # --- Auto-Restart Supervisor Loop ---
    # If the bot crashes because of a network issue, Telegram error, or any
    # unexpected exception, it waits a few seconds and starts again automatically.
    RESTART_DELAY = int(os.getenv('RESTART_DELAY', 10))  # seconds between restarts
    restart_count = 0
    ...
    while True:
        try:
            ...
            if not TOKEN or TOKEN == "YOUR_FALLBACK_BOT_TOKEN":
                ...
                if restart_count > 0:
                    print(f"\n♻️ Restarting bot (attempt #{restart_count})...")
                    logger.info(f"Auto-restart attempt #{restart_count} starting.")
                asyncio.run(main())
                # If main() returns normally, restart anyway to keep the bot alive
                print("\n⚠️ Bot main loop exited normally. Restarting to stay online...")
                logger.warning("main() returned; auto-restarting.")
        except KeyboardInterrupt:
            print("\n-- Bot stopping due to Ctrl+C --")
            logger.info("Bot stopped manually via KeyboardInterrupt.")
            break
        except SystemExit as se:
            # exit(1) inside main() raises SystemExit; treat it as a crash and restart
            print(f"\n⚠️ Bot exited (code {se.code}). Auto-restarting...")
            logger.error(f"SystemExit caught (code {se.code}); auto-restarting.")
        except Exception as e:
